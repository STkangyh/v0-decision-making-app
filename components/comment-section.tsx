...
async function fetchComments(decisionId: string): Promise<Comment[]> {
  ...
  const { data, error } = await fetchCommentsForDecision(supabase, decisionId)
...
export function CommentSection({ decisionId }: CommentSectionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await insertComment(supabase, {
        decisionId,
        sessionId,
  ...
  const handleDelete = async (commentId: string) => {
    try {
      ...
      const { error } = await deleteCommentForSession(supabase, {
        commentId,
        sessionId,
      })
          ...
          {comments.map((comment) => (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                ...
                {comment.session_id === getSessionId() && (
                  <button
